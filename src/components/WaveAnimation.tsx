  ...
  useEffect(() => {
    ...
    const updateDom = () => {
      ...
      for (let y = minY; y < 100; y += delta += deltaDelta += 0.01) {
        ...
        const rA = 100 / 1.5;
        const thetaA =
          Math.PI * (-0.15 + 0.3 + 0.3 * Math.sin(y / 150 + 0.6 + n / 100));
        const rB = 100 / 2;
        const thetaB =
          Math.PI * (-0.2 + 0.3 + 0.3 * Math.sin(y / 150 + 1.3 + n / 110));
        const d =
          "M" +
          pA[0] +
          "," +
      ...
      for (let x = 0; x < maxX; x += delta += deltaDelta += 0.01) {
        ...
        const rA = 100 / 3;
        const thetaA =
          Math.PI * (0.6 + 0.15 * Math.sin(x / 150 + 0.3 + n / 120));
        const rB = 100 / 3;
        const thetaB =
          Math.PI * (0.5 + 0.15 * Math.sin(x / 150 + 1.9 + n / 200));
        const d =
          "M" +
          (100 - pA[0]) +
          "," +
